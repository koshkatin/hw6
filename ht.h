...
{
    // Calculate load factor, including deleted items
    double alpha = static_cast<double>(numItems_ + numDeleted_) / table_.size();
    if (alpha >= resizeAlpha_) {
        resize();
    }

    // Use the probe function to find the correct location for the key
    HASH_INDEX_T loc = probe(p.first);
    if (loc == npos) {
        throw std::logic_error("No available slot for insert");
    }
...
void HashTable<K,V,Prober,Hash,KEqual>::resize()
{
    ...
    table_.resize(CAPACITIES[mIndex_], nullptr);
    numItems_ = 0;
    numDeleted_ = 0; // Reset the count of deleted items
...
template<typename K, typename V, typename Prober, typename Hash, typename KEqual>
size_t HashTable<K, V, Prober, Hash, KEqual>::tableSize() const
{
    return table_.size();
}
